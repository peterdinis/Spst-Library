...
interface AuthorFormProps {
  initialData?: { id: string; name: string; bio?: string | null; imageUrl?: string | null };
  onSuccess?: () => void;
}
  ...
  return (
    <Card className="shadow-lg border-slate-200/80 dark:border-slate-800 rounded-2xl overflow-hidden bg-white dark:bg-slate-900">
      <CardHeader className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/50">
        <CardTitle className="text-xl">{isEditing ? "Upraviť autora" : "Nový autor"}</CardTitle>
        <CardDescription>
      ...
      <CardContent className="pt-6 space-y-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Meno</label>
            <Input
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Životopis</label>
            <Textarea
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Fotka autora</label>
            <FileUpload
            ...
            {imageUrl ? (
              <div className="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-700 p-3 bg-slate-50 dark:bg-slate-800/50">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate flex-1">{imageUrl}</p>
