                ...
                <TableCell className="text-right pr-6 space-x-2">
                  <Dialog open={isEditDialogOpen && selectedAuthor?.id === a.id} onOpenChange={(open) => {
                    ...
                    <DialogContent className="sm:max-w-md rounded-3xl">
                      <AuthorForm initialData={a} onSuccess={() => setIsEditDialogOpen(false)} />
                    </DialogContent>
                  </Dialog>
                  <Button 
                    variant="ghost" 
      ...
      <CardContent className="p-0">
        <Table>
          ...
          <TableBody>
              ...
              <TableRow key={c.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <TableCell className="text-right pr-6 space-x-2">
                  <Dialog open={isEditDialogOpen && selectedCategory?.id === c.id} onOpenChange={(open) => {
                    ...
                    <DialogContent className="sm:max-w-md rounded-3xl">
                      <CategoryForm initialData={c} onSuccess={() => setIsEditDialogOpen(false)} />
                    </DialogContent>
                  </Dialog>
                  <Button 
                    variant="ghost" 
            ...
            {!categories?.length && (
              <TableRow>
                <TableCell colSpan={2} className="text-center py-12 text-slate-400">Žiadne kategórie neboli nájdené.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
              ...
              <TableRow key={b.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors group">
                ...
                <TableCell className="font-medium">
                    <div>
                        ...
                        <div className="text-xs text-slate-500 font-normal">{b.isbn || "ISBN chýba"}</div>
                    </div>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                    <div className="flex items-center gap-2 font-mono text-sm">
                        <span className="text-indigo-600 font-bold">{b.availableCopies}</span>
                    </div>
                </TableCell>
                <TableCell className="text-right pr-6 space-x-2">
                  <Dialog open={isEditDialogOpen && selectedBook?.id === b.id} onOpenChange={(open) => {
                    setIsEditDialogOpen(open);
                    if (open) setSelectedBook(b);
                  }}>
                    <DialogTrigger>
                      <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 border border-transparent hover:border-indigo-100">
                        <Edit className="h-4 w-4" />
                      </Button>
                    </DialogTrigger>
                  ...
                  <Button 
                    ...
                    onClick={() => {
                        if(confirm(`Naozaj chcete vymazať knihu "${b.title}"?`)) {
