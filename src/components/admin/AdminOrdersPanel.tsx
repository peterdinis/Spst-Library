...
export function AdminOrdersPanel() {
  ...
  if (isLoading) {
    return <p className="text-sm text-muted-foreground">Načítavam objednávky…</p>;
  }
  ...
  if (!orders?.length) {
    return <p className="text-sm text-muted-foreground">Zatiaľ žiadne objednávky.</p>;
  }
              ...
              <TableCell>
                <Select
                  ...
                  onValueChange={(status) =>
                    updateStatus.mutate({
                      id: row.id,
                      status: status as "pending" | "approved" | "fulfilled" | "cancelled",
                    })
                  }
