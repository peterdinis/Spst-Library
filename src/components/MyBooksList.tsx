...
export function MyBooksList() {
  const { data: books, isLoading, refetch } = trpc.books.getBorrowedByUser.useQuery();
  ...
  if (!books?.length) {
    return (
      <div className="text-center p-12 bg-card rounded-xl border shadow-sm">
        ...
        <p className="text-slate-500 mb-6">Zatiaľ ste si nepožičali žiadnu knihu. Pozrite si náš katalóg!</p>
      </div>
    );
  }
      ...
      {books.map((record) => (
        <Card key={record.id} className="flex flex-col">
          <CardHeader>
            ...
            <CardDescription>Termín vrátenia: {new Date(record.dueDate).toLocaleDateString("sk-SK")}</CardDescription>
          </CardHeader>
          <CardContent className="flex-1">
            <p className="text-sm">
              <span className="font-medium">Vypožičané:</span> {new Date(record.borrowDate).toLocaleDateString("sk-SK")}
            </p>
          </CardContent>
          <CardFooter>
            <Button 
              ...
              onClick={() => execute({ borrowId: record.id, bookId: record.bookId })}
            >
              Vrátiť knihu
            </Button>
          </CardFooter>
