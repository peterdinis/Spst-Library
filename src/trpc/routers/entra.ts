...
export const entraRouter = router({
  ...
  listDirectoryUsers: publicProcedure.query(async () => {
    try {
      ...
      const message = e instanceof Error ? e.message : "Nepodarilo sa načítať používateľov z Entra.";
      return { users: [] as Awaited<ReturnType<typeof fetchEntraDirectoryUsers>>, error: message };
    }
  }),
});
