  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        if (!credentials?.username || !credentials?.password || !credentials?.adminCode) {
          return null;
        }

        const admin = db.select().from(admins).where(eq(admins.username, credentials.username as string)).get();
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      if (account?.provider === "microsoft-entra-id") {
        ...
        const existingUser = db.select().from(users).where(eq(users.email, user.email)).get();
        if (!existingUser) {
          ...
          db.update(users).set({ 
            name: user.name, 
            image: user.image 
          }).where(eq(users.email, user.email)).run();
        }
      }
      return true;
    },
