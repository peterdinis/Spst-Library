...
import { ArrowRight, BookOpen, Search, Star, Library, Users } from "lucide-react";
        ...
        <div className="flex w-full justify-between items-end mb-10">
          <div>
            <div className="flex items-center gap-2 mb-2 text-primary">
              ...
              <span className="uppercase tracking-widest font-bold text-sm">Novinky</span>
            </div>
            <h2 className="text-4xl font-extrabold tracking-tight">Najnovšie prírastky</h2>
          </div>
          <Link href="/books">
            <Button variant="ghost" className="hidden sm:flex rounded-full group">
              Zobraziť všetky <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
            </Button>
          </Link>
        </div>
              ...
              <Card className="h-full bg-card/50 backdrop-blur-sm border-slate-200/50 hover:border-primary/50 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 rounded-3xl overflow-hidden">
                <div className="aspect-[3/4] relative overflow-hidden bg-slate-100 dark:bg-slate-900">
                  ...
                  <div className="absolute top-4 left-4 bg-background/90 backdrop-blur-md px-3 py-1 rounded-full border border-slate-200/20 shadow-sm">
                    <span className="text-xs font-bold uppercase tracking-wider text-primary">{book.category?.name || "Kniha"}</span>
                  </div>
                </div>
                <CardContent className="p-6">
                  <h3 className="font-bold text-xl mb-1 group-hover:text-primary transition-colors line-clamp-1">{book.title}</h3>
                  <p className="text-slate-500 text-sm font-medium">{(book as any).author?.name || "Neznámy autor"}</p>
                </CardContent>
              </Card>
            </Link>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-24">
        ...
        <section>
          <div className="flex items-center gap-2 mb-2 text-primary">
            ...
            <span className="uppercase tracking-widest font-bold text-sm">Žánre</span>
          </div>
          <div className="flex w-full justify-between items-end mb-8">
            <h2 className="text-3xl font-extrabold tracking-tight">Populárne Kategórie</h2>
            <Link href="/categories">
              <Button variant="link" className="text-primary p-0">Všetky</Button>
            </Link>
          </div>
        ...
        <section>
          <div className="flex items-center gap-2 mb-2 text-primary">
            ...
            <span className="uppercase tracking-widest font-bold text-sm">Spisovatelia</span>
          </div>
          <div className="flex w-full justify-between items-end mb-8">
            <h2 className="text-3xl font-extrabold tracking-tight">Obľúbení Autori</h2>
            <Link href="/authors">
              <Button variant="link" className="text-primary p-0">Všetci</Button>
            </Link>
          </div>
              ...
              <Link href={`/authors/${author.id}`} key={author.id}>
                <Card className="bg-card/50 backdrop-blur-sm hover:bg-card transition-colors border-slate-200/50 hover:border-primary/30 rounded-2xl group hover:shadow-md">
                  <CardHeader className="p-5 flex flex-row items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-lg group-hover:text-primary transition-colors">{author.name}</CardTitle>
                      <p className="text-sm text-slate-500 line-clamp-1">{author.bio}</p>
                    </div>
                  </CardHeader>
                </Card>
              </Link>
