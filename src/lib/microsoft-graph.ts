...
function getGraphCredentials() {
  const tenantId =
    process.env.AUTH_MICROSOFT_ENTRA_ID_TENANT_ID ?? process.env.AZURE_AD_TENANT_ID;
  const clientId =
    process.env.AUTH_MICROSOFT_ENTRA_ID_CLIENT_ID ?? process.env.AZURE_AD_CLIENT_ID;
  const clientSecret =
    process.env.AUTH_MICROSOFT_ENTRA_ID_CLIENT_SECRET ?? process.env.AZURE_AD_CLIENT_SECRET;
...
async function getGraphAccessToken(): Promise<string> {
  ...
  if (!res.ok || !data.access_token) {
    throw new Error(data.error_description || `Graph token: ${res.status} ${res.statusText}`);
  }
  return data.access_token;
}

export async function fetchEntraDirectoryUsers(): Promise<EntraDirectoryUser[]> {
