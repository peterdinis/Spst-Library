...
export const createBookAction = protectedActionClient
  ...
  .action(async ({ parsedInput: { title, description, isbn, availableCopies, authorId, categoryId } }) => {
    const id = crypto.randomUUID();
    db.insert(books).values({
      id, title, description, isbn, availableCopies, authorId, categoryId
    }).run();
...
export const borrowBookAction = protectedActionClient
  ...
  .action(async ({ parsedInput: { bookId }, ctx: { session } }) => {
    ...
    const book = db.select({ availableCopies: books.availableCopies }).from(books).where(eq(books.id, bookId)).get();
    ...
    db.transaction((tx) => {
      tx.insert(borrowedBooks).values({
...
export const returnBookAction = protectedActionClient
  ...
  .action(async ({ parsedInput: { borrowId, bookId }, ctx: { session } }) => {
    const userId = session?.user?.id;
    if (!userId) throw new Error("Unauthorized");
    const record = db.select().from(borrowedBooks).where(and(eq(borrowedBooks.id, borrowId), eq(borrowedBooks.userId, userId))).get();
    ...
    db.transaction((tx) => {
      tx.update(borrowedBooks)
        ...
        .where(eq(borrowedBooks.id, borrowId)).run();
