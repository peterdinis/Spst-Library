    ...
    try {
      ...
      await db.transaction(async tx => {
        ...
        await tx.insert(notifications).values({
          id: crypto.randomUUID(),
          userId: userId,
          message: `Úspešne ste si požičali knihu "${
            book.title
          }". Termín vrátenia je ${dueDate.toLocaleDateString('sk-SK')}.`,
          type: 'borrow',
          isRead: false
        })
      })
        ...
        try {
          await sendTransactionalEmail(
            ...
            `
					...
					<p>Termín vrátenia je: <strong>${dueDate.toLocaleDateString(
            'sk-SK'
          )}</strong>.</p>
      ...
      if (e instanceof Error) {
        throw new Error('DETAIL CHYBY: ' + e.message)
      } else {
        throw new Error('DETAIL CHYBY: ' + String(e))
      }
    }
  })
...
export const clearReturnHistoryAction = protectedActionClient
