...
const AuthorForm: FC<AuthorFormProps> = ({ initialData, onSubmit, onCancel, isLoading = false }) => {
    const {
        register,
        handleSubmit,
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>{initialData?.id ? 'Upraviť autora' : 'Pridať nového autora'}</CardTitle>
                <CardDescription>
                    {initialData?.id ? 'Upravte informácie o autorovi' : 'Vyplňte informácie o novom autorovi'}
                </CardDescription>
            </CardHeader>
