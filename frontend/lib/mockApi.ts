...
export const mockBooksApi = {
    ...
    updateAvailability: (bookId: string, change: number) => {
        ...
        if (book) {
            const updatedBook = {
                ...book,
                availableCopies: Math.max(0, Math.min(book.totalCopies, book.availableCopies + change))
            };
...
export const mockBorrowApi = {
    borrowBook: async (userId: string, bookId: string): Promise<BorrowRecord> => {
        ...
        const activeBorrow = records.find((r: BorrowRecord) =>
    ...
    getActiveBorrows: async (userId: string): Promise<BorrowRecord[]> => {
        await delay(300);
        const records = borrowStorage.getRecords();
        return records.filter((r: BorrowRecord) =>
            r.userId === userId && r.status === 'active'
        );
    },

    checkUserHasBorrowed: async (userId: string, bookId: string): Promise<boolean> => {
        const records = borrowStorage.getRecords();
        return records.some((r: BorrowRecord) =>
            r.userId === userId && r.bookId === bookId && r.status === 'active'
        );
    },
};
