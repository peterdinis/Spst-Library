...
import { CreateBookData, UpdateBookData, CreateAuthorData, UpdateAuthorData, CreateCategoryData, UpdateCategoryData } from './validations';
...
export const mockBooksCrudApi = {
    ...
    update: async (data: UpdateBookData): Promise<Book> => {
        ...
        if (data.isbn && data.isbn !== existingBook.isbn) {
            const duplicateBook = booksData.find(b => b.isbn === data.isbn && b.id !== data.id);
            if (duplicateBook) {
                throw new Error('Kniha s týmto ISBN už existuje');
            }
        }
...
export const mockAuthorsCrudApi = {
    ...
    create: async (data: CreateAuthorData): Promise<Author> => {
        ...
        const existingAuthor = authorsData.find(a => a.name.toLowerCase() === data.name.toLowerCase());
        if (existingAuthor) {
    ...
    update: async (data: UpdateAuthorData): Promise<Author> => {
        ...
        if (data.name && data.name !== existingAuthor.name) {
            const duplicateAuthor = authorsData.find(
                a => a.name.toLowerCase() === data.name!.toLowerCase() && a.id !== data.id
            );
            if (duplicateAuthor) {
                throw new Error('Autor s týmto menom už existuje');
            }
    ...
    delete: async (id: string): Promise<void> => {
        ...
        if (hasBooks) {
            throw new Error('Nemôžete vymazať autora, ktorý má knihy. Najprv vymažte všetky jeho knihy.');
        }
...
export const mockCategoriesCrudApi = {
    ...
    update: async (data: UpdateCategoryData): Promise<Category> => {
        ...
        const index = categoriesData.findIndex(category => category.id === data.id);
        if (index === -1) {
    ...
    delete: async (id: string): Promise<void> => {
        ...
        if (hasBooks) {
            throw new Error('Nemôžete vymazať kategóriu, ktorá má knihy. Najprv presuňte alebo vymažte všetky knihy.');
        }
